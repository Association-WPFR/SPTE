...
- README.md, ligne de badges en tête (`<p align="center">…</p>`) : les badges sont en markdown pur (`[![alt](img)](lien)`), générés par `istanbul-badges-readme` qui ne sait émettre que ça (jamais de `<img>`). Du markdown mélangé à du HTML brut sur les mêmes lignes ne se rend PAS sur GitHub (affiché en texte littéral) — le contenu markdown doit être dans son propre paragraphe, isolé par une ligne vide avant et après, même à l'intérieur du `<p>`. Ne pas revenir à des `<img>`/`<a>` HTML mêlés au marqueur `#lines#` sans cette précaution.
